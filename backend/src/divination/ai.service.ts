...
export interface NamingAiResult {
  ...
  tuijian: { name: string; pinyin: string; analysis: string; youlai: string; diangu: string; jianjie: string }[]; // 推荐名字
  jianyi: string[];        // 取名建议
}
...
export class AiService {
  ...
  /** 调用 AI 解读八字 */
  async interpret(bazi: BaZiResult, name: string, gender: number, question?: string): Promise<AiInterpretation> {
  ...
  private buildPrompt(bazi: BaZiResult, name: string, gender: string, question?: string): string {
    const { siZhu, wuXingCount, wuXingLack, riZhu, riZhuWuXing, shenQiangRuo, yongShen, jiShen, geJu, lunarInfo } = bazi;
    return `请为以下命主进行八字详批：
...
以 JSON 格式返回（不要包含 markdown 代码块标记）：
{
  ...
  /** 紫微斗数 AI 解读 */
  async interpretZiwei(ziwei: ZiweiResult, name: string, birthplace?: string, lat?: number, lng?: number): Promise<ZiweiAiResult> {
    const palaceInfo = ziwei.palaces.map(p => {
      const majors = p.majorStars.map(s => `${s.name}(${s.brightness})`).join('、') || '无主星';
...
请以 JSON 格式返回分析结果（不要包含 markdown 代码块标记）：
{
  ...
  private ziweiFallback(ziwei: ZiweiResult): ZiweiAiResult {
    return {
      ...
      liunian: '流年运势待 AI 详细分析。',
      yanzheng: '',
      advice: { suitable: ['稳步前行'], avoid: ['冲动决策'], luckyDirection: '南方', luckyElement: '火' },
    };
  }

  /** 奇门遁甲 AI 解读 */
  async interpretQimen(qimen: QimenResult, name: string): Promise<QimenAiResult> {
    const palaceInfo = qimen.palaces.map(p =>
    ...
    const auspStr = qimen.auspicious.map(a => `${a.name}(${a.type}) - ${a.description}`).join('\n') || '无';
    const inausStr = qimen.inauspicious.map(a => `${a.name}(${a.type}) - ${a.description}`).join('\n') || '无';
...
以 JSON 格式返回（不要包含 markdown 代码块标记）：
{
  ...
  /** 六爻 AI 解读 */
  async interpretLiuyao(liuyao: LiuyaoResult, question: string): Promise<LiuyaoAiResult> {
    const o = liuyao.origin;
    const c = liuyao.changed;
    const movingDesc = liuyao.movingIndexes.length > 0
      ? liuyao.movingIndexes.map(i => o.lineTexts[i]?.label).join('、') + ' 动'
      : '无动爻（静卦）';

    const yaoLines = o.ganzhi.map((gz, i) => {
      const moving = liuyao.movingIndexes.includes(i) ? ' [动]' : '';
      const shiying = i === o.shi - 1 ? ' [世]' : i === o.ying - 1 ? ' [应]' : '';
...
以 JSON 格式返回（不要包含 markdown 代码块标记）：
{
  ...
  /** 易经占卜 AI 解读 */
  async interpretYijing(yijing: YijingResult, question: string): Promise<YijingAiResult> {
    const o = yijing.origin;
    const yaoDesc = yijing.yaos.map((y, i) =>
    ...
    const focusInfo = yijing.focus.length > 0 ? yijing.focus.join('\n') : '以本卦卦辞为主';
...
以 JSON 格式返回（不要包含 markdown 代码块标记）：
{
  ...
  /** 小六壬 AI 解读 */
  async interpretXiaoliuren(result: XiaoliurenResult): Promise<XiaoliurenAiResult> {
...
以 JSON 格式返回（不要包含 markdown 代码块标记）：
{
  ...
  private xunwuFallback(result: any): XunwuAiResult {
    ...
    return {
      ...
      keneng: result.canFind ? `卦象吉利，${typeText}有望寻回。建议尽快按方位寻找。` : `卦象不利，${typeText}寻找较难。但仍需尽力而为，不可轻言放弃。`,
      jianyi: [
  ...
  async interpretZeji(result: ZejiResult): Promise<ZejiAiResult> {
    const bestInfo = result.bestDays.slice(0, 5).map(d =>
...
以 JSON 格式返回（不要包含 markdown 代码块标记）：
{
  ...
  private zejiFallback(result: ZejiResult): ZejiAiResult {
    ...
    return {
      zonglun: best ? `查询范围内共找到${result.bestDays.length}个吉日，最佳日期为${best.date}（${best.lunarDate}），${best.jianXing}日，黄道吉日。` : '查询范围内未找到完全匹配的吉日，建议扩大查询范围。',
      tuijian: '推荐日期待 AI 详细分析。',
      yiji: '宜忌待 AI 详细分析。',
      chongsha: best ? `${best.chongSha}，请注意回避。` : '冲煞待 AI 详细分析。',
      zhuyi: '注意事项待 AI 详细分析。',
  ...
  async interpretNaming(result: NamingResult): Promise<NamingAiResult> {
    ...
    const wxStr = Object.entries(bazi.wuxingCount).map(([k, v]) => `${k}${v}`).join(' ');
    const lackStr = bazi.wuxingLack.length > 0 ? bazi.wuxingLack.join('、') : '无';
    const strokesStr = result.recommendStrokes.slice(0, 5).map(s => `${s[0]}+${s[1]}画`).join('、');
...
以 JSON 格式返回（不要包含 markdown 代码块标记）：
{
  ...
  /** 起名改名 AI 解读（测名模式） */
  async interpretNamingCheck(result: NamingResult, name: string): Promise<NamingCheckAiResult> {
    const bazi = result.bazi;
    const wuge = result.wuge!;
    const wxStr = Object.entries(bazi.wuxingCount).map(([k, v]) => `${k}${v}`).join(' ');
    const nameCharsStr = result.nameChars!.map(c => `${c.char}(${c.stroke}画)`).join(' ');
...
以 JSON 格式返回（不要包含 markdown 代码块标记）：
{
  ...
  async interpretHehun(hehun: HehunResult): Promise<HehunAiResult> {
    ...
    const mWxStr = Object.entries(m.wuXingCount).map(([k, v]) => `${k}${v}`).join(' ');
    const fWxStr = Object.entries(f.wuXingCount).map(([k, v]) => `${k}${v}`).join(' ');
...
以 JSON 格式返回（不要包含 markdown 代码块标记）：
{
  ...
  private shengyiFallback(data: any): ShengyiAiResult {
    ...
    return {
      ...
      jianyi: ['明确分工合作', '发挥各自优势', '注意五行互补', '定期沟通协调', '建立决策机制'],
    };
  }
  ...
  /** 获取指定类型的 prompt 配置 */
  getPromptConfig(type: string, data: any, extraParams?: any): { system: string; user: string; maxTokens: number; timeout: number; temperature: number } {
    switch (type) {
      case 'bazi': {
        const genderStr = extraParams?.gender === 1 ? '男' : '女';
        return { system: SYSTEM_PROMPT, user: this.buildPrompt(data, extraParams?.name ?? '', genderStr, extraParams?.question), maxTokens: 4000, timeout: 60000, temperature: 0.7 };
      }
      case 'meihua': {
        const m = data;
        ...
        return { system: MEIHUA_SYSTEM_PROMPT, user, maxTokens: 2000, timeout: 60000, temperature: 0.7 };
      }
      case 'ziwei': {
        const z = data;
        const palaceInfo = z.palaces.map((p: any) => {
          const majors = p.majorStars.map((s: any) => `${s.name}(${s.brightness})`).join('、') || '无主星';
          const minors = p.minorStars.length > 0 ? p.minorStars.join('、') : '无';
          return `${p.name}（${p.heavenlyStem}${p.earthlyBranch}）：主星[${majors}] 辅星[${minors}]`;
        }).join('\n');
        ...
        return { system: ZIWEI_SYSTEM_PROMPT, user, maxTokens: 4000, timeout: 60000, temperature: 0.7 };
      }
      case 'qimen': {
        const q = data;
        const palaceInfo = q.palaces.map((p: any) => `${p.position}宫(${p.trigram}): 天盘${p.heavenlyStem} 地盘${p.earthlyStem} ${p.star}(${p.starStatus}) ${p.gate}(${p.gateStatus}) ${p.deity}${p.isZhiFu ? ' [值符]' : ''}${p.isZhiShi ? ' [值使]' : ''}${p.gatePressure !== '无' ? ' [门迫]' : ''}`).join('\n');
        const auspStr = q.auspicious.map((a: any) => `${a.name}(${a.type}) - ${a.description}`).join('\n') || '无';
        const inausStr = q.inauspicious.map((a: any) => `${a.name}(${a.type}) - ${a.description}`).join('\n') || '无';
        ...
        return { system: QIMEN_SYSTEM_PROMPT, user, maxTokens: 4000, timeout: 60000, temperature: 0.7 };
      }
      case 'liuyao': {
        const l = data;
        const o = l.origin;
        const c = l.changed;
        const movingDesc = l.movingIndexes.length > 0 ? l.movingIndexes.map((i: number) => o.lineTexts[i]?.label).join('、') + ' 动' : '无动爻（静卦）';
        const yaoLines = o.ganzhi.map((gz: string, i: number) => {
        ...
        return { system: LIUYAO_SYSTEM_PROMPT, user, maxTokens: 4000, timeout: 60000, temperature: 0.7 };
      }
      case 'yijing': {
        const y = data;
        const o = y.origin;
        const yaoDesc = y.yaos.map((ya: any, i: number) => `${ya.position}（${ya.value === 9 ? '老阳' : ya.value === 6 ? '老阴' : ya.value === 7 ? '少阳' : '少阴'}${ya.isMoving ? '·动' : ''}）：${ya.text}`).reverse().join('\n');
        const changedInfo = y.changed ? `\n【变卦】${y.changed.symbol} ${y.changed.name}\n卦辞：${y.changed.judgement}\n大象：${y.changed.text}` : '';
        const focusInfo = y.focus.length > 0 ? y.focus.join('\n') : '以本卦卦辞为主';
        ...
        return { system: YIJING_SYSTEM_PROMPT, user, maxTokens: 4000, timeout: 60000, temperature: 0.7 };
      }
      case 'xiaoliuren': {
        const x = data;
        ...
        return { system: XIAOLIUREN_SYSTEM_PROMPT, user, maxTokens: 2000, timeout: 60000, temperature: 0.7 };
      }
      case 'xunwu': {
        const x = data;
        const typeText = x.type === 'xunwu' ? '寻物' : '寻人';
        const targetText = x.type === 'xunwu' ? `【物品】${x.itemName || '未说明'}` : `【人物】${x.personName || '未说明'}`;
        ...
        return { system: XUNWU_SYSTEM_PROMPT, user, maxTokens: 2000, timeout: 60000, temperature: 0.7 };
      }
      case 'zeji': {
        const r = data;
        const bestInfo = r.bestDays.slice(0, 5).map((d: any) => `${d.date}（${d.lunarDate} ${d.weekday}）干支：${d.ganzhi} 建星：${d.jianXing} ${d.isHuangDao ? '黄道' : '黑道'} 评分：${d.score}\n  宜：${d.yi.slice(0, 8).join('、')}\n  忌：${d.ji.slice(0, 6).join('、')}\n  ${d.chongSha}`).join('\n\n') || '未找到匹配的吉日';
        ...
        return { system: ZEJI_SYSTEM_PROMPT, user, maxTokens: 2000, timeout: 60000, temperature: 0.7 };
      }
      case 'naming': {
        const r = data;
        const bazi = r.bazi;
        const wxStr = Object.entries(bazi.wuxingCount).map(([k, v]) => `${k}${v}`).join(' ');
        const lackStr = bazi.wuxingLack.length > 0 ? bazi.wuxingLack.join('、') : '无';
        const strokesStr = r.recommendStrokes.slice(0, 5).map((s: any) => `${s[0]}+${s[1]}画`).join('、');
        ...
        return { system: NAMING_SYSTEM_PROMPT, user, maxTokens: 5000, timeout: 120000, temperature: 0.8 };
      }
      case 'namingCheck': {
        const r = data;
        const bazi = r.bazi;
        const wuge = r.wuge;
        const wxStr = Object.entries(bazi.wuxingCount).map(([k, v]) => `${k}${v}`).join(' ');
        const nameCharsStr = r.nameChars.map((c: any) => `${c.char}(${c.stroke}画)`).join(' ');
        ...
        return { system: NAMING_SYSTEM_PROMPT, user, maxTokens: 3000, timeout: 120000, temperature: 0.7 };
      }
      case 'hehun': {
        const h = data;
        const m = h.male.bazi;
        const f = h.female.bazi;
        const mWxStr = Object.entries(m.wuXingCount).map(([k, v]: [string, any]) => `${k}${v}`).join(' ');
        const fWxStr = Object.entries(f.wuXingCount).map(([k, v]: [string, any]) => `${k}${v}`).join(' ');
        ...
        return { system: HEHUN_SYSTEM_PROMPT, user, maxTokens: 4000, timeout: 120000, temperature: 0.7 };
      }
      case 'shengyi': {
        const s = data;
        const self = s.self;
        const partners = s.partners;
        const selfWxStr = Object.entries(self.bazi.wuXingCount).map(([k, v]: [string, any]) => `${k}${v}`).join(' ');
        const partnersInfo = partners.map((p: any) => {
          const pWxStr = Object.entries(p.bazi.wuXingCount).map(([k, v]: [string, any]) => `${k}${v}`).join(' ');
          return `【合伙人】${p.name}
八字：${p.bazi.rawBaZi}
...
{"zonglun":"合伙总论，300字左右","hehuo":"合伙优势详细分析，250字左右","fengxian":"风险提示与注意事项，250字左右","bazi":"八字配合深度解读，200字左右","hangye":"行业适合度与五行调整建议，200字左右","lingdao":"领导力分工与权力平衡建议，200字左右","jianyi":["合作建议1","合作建议2","合作建议3","合作建议4","合作建议5"]}`;
        return { system: SHENGYI_SYSTEM_PROMPT, user, maxTokens: 4000, timeout: 120000, temperature: 0.7 };
      }
      case 'face': {
        const genderInfo = data.gender && data.gender !== '未知' ? `\n性别：${data.gender}` : '';
        ...
        return { system: FACE_SYSTEM_PROMPT, user, maxTokens: 4000, timeout: 120000, temperature: 0.7 };
      }
      case 'palm': {
        ...
        return { system: PALM_SYSTEM_PROMPT, user, maxTokens: 4000, timeout: 120000, temperature: 0.7 };
      }
      case 'tongue': {
        const genderInfo = data.gender && data.gender !== '未知' ? `\n性别：${data.gender}` : '';
        const birthInfo = data.birthYear ? `\n出生年份：${data.birthYear}年（${new Date().getFullYear() - data.birthYear}岁左右）` : '';
        ...
        return { system: TONGUE_SYSTEM_PROMPT, user, maxTokens: 4000, timeout: 120000, temperature: 0.7 };
      }
      case 'fengshui': {
        const dirInfo = data.direction && data.direction !== '未知' ? `\n房屋朝向：${data.direction}` : '';
        const typeInfo = data.houseType && data.houseType !== '未知' ? `\n户型描述：${data.houseType}` : '';
        const genderInfo = data.ownerGender && data.ownerGender !== '未知' ? `\n户主性别：${data.ownerGender}` : '';
        ...
        return { system: FENGSHUI_SYSTEM_PROMPT, user, maxTokens: 4000, timeout: 120000, temperature: 0.7 };
      }
      case 'chouqian': {
        const sign = data;
...
以 JSON 格式返回（不要包含 markdown 代码块标记）：
{
  ...
  "jianyi": ["具体建议1", "具体建议2", "具体建议3", "具体建议4"]
        ...
        return { system: CHOUQIAN_SYSTEM_PROMPT, user, maxTokens: 3000, timeout: 60000, temperature: 0.7 };
      }
      case 'dream': {
...
以 JSON 格式返回（不要包含 markdown 代码块标记）：
{
  ...
  "jianyi": ["具体建议1", "具体建议2", "具体建议3", "具体建议4"]
        ...
        return { system: DREAM_SYSTEM_PROMPT, user, maxTokens: 4000, timeout: 60000, temperature: 0.7 };
      }
      default:
  ...
  async *streamInterpretation(system: string, user: string, maxTokens: number, timeout: number, temperature: number): AsyncGenerator<string> {
  ...
  async *streamVisionInterpretation(system: string, userText: string, imageBase64: string, maxTokens: number, timeout: number, temperature: number): AsyncGenerator<string> {
    const stream = await this.client.chat.completions.create({
  ...
  getFallback(type: string, data: any): any {
    switch (type) {
      case 'bazi': return this.fallbackResult(data);
      case 'meihua': return this.meihuaFallback(data);
      case 'ziwei': return this.ziweiFallback(data);
      case 'qimen': return this.qimenFallback(data);
      case 'liuyao': return this.liuyaoFallback(data);
      case 'yijing': return this.yijingFallback(data);
      case 'xiaoliuren': return this.xiaoliurenFallback(data);
      case 'xunwu': return this.xunwuFallback(data);
      case 'zeji': return this.zejiFallback(data);
      case 'naming': return this.namingFallback(data);
      case 'namingCheck': return this.namingCheckFallback(data);
      case 'hehun': return this.hehunFallback(data);
      case 'shengyi': return this.shengyiFallback(data);
      case 'face': return this.faceFallback();
      case 'palm': return this.palmFallback();
      case 'fengshui': return this.fengshuiFallback();
      case 'chouqian': return this.chouqianFallback(data);
      case 'dream': return this.dreamFallback(data);
      default: return {};
    }
  }
  ...
  private fengshuiFallback(): FengshuiAiResult {
    return {
      ...
      shaye: '煞气分析待 AI 视觉分析。',
      buju: '布局建议待 AI 视觉分析。',
      jianyi: ['保持室内整洁通风', '注意大门纳气方向', '合理摆放家具', '适当添加绿植'],
    };
  }
}
