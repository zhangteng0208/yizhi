...
function getShuli(n: number): { ji: boolean; desc: string } {
  ...
  return (
    JIXIONG_81[mod] || { ji: mod % 2 === 1, desc: mod % 2 === 1 ? '吉' : '凶' }
  );
}
...
export class NamingService {
  ...
  calculate(
    ...
    if (name && name.length > 0) {
      const nameChars = [...name].map((c) => ({
        char: c,
        stroke: getKangxiStroke(c),
      }));
      const wuge = this.calcWuge(
        surnameStrokes,
        nameChars.map((c) => c.stroke),
      );
  ...
  private getBazi(
    ...
    const wuxingLack = Object.entries(wuxingCount)
      .filter(([, v]) => v === 0)
      .map(([k]) => k);
  ...
  private calcWuge(surnameStrokes: number, nameStrokes: number[]): WugeResult {
    ...
    const digeVal =
      nameStrokes.length === 1
        ? firstNameStroke + 1
        : firstNameStroke + lastNameStroke;
    const zonggeVal = surnameStrokes + totalName;
    const waigeVal = nameStrokes.length === 1 ? 2 : zonggeVal - rengeVal + 1;
    ...
    return {
      tiange: {
        value: tiangeVal,
        wuxing: BIHUA_WUXING(tiangeVal),
        shuli: tiangeShuli.desc,
        ji: tiangeShuli.ji,
      },
      renge: {
        value: rengeVal,
        wuxing: BIHUA_WUXING(rengeVal),
        shuli: rengeShuli.desc,
        ji: rengeShuli.ji,
      },
      dige: {
        value: digeVal,
        wuxing: BIHUA_WUXING(digeVal),
        shuli: digeShuli.desc,
        ji: digeShuli.ji,
      },
      waige: {
        value: Math.max(1, waigeVal),
        wuxing: BIHUA_WUXING(Math.max(1, waigeVal)),
        shuli: waigeShuli.desc,
        ji: waigeShuli.ji,
      },
      zongge: {
        value: zonggeVal,
        wuxing: BIHUA_WUXING(zonggeVal),
        shuli: zonggeShuli.desc,
        ji: zonggeShuli.ji,
      },
      sancai,
    };
