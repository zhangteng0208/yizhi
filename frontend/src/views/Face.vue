  ...
  <div class="page">
    ...
    <div class="form" v-if="!result">
      <div class="card upload-card">
        <h3 class="card-title">上传面部照片</h3>
        <p class="upload-hint">请上传正面清晰的面部照片，光线充足效果更佳</p>
        <van-uploader
          v-model="fileList"
          :max-count="1"
    ...
    <template v-if="result">
      ...
      <div class="card summary-card" v-if="result.ai">
        <div class="summary-label">面相综合评分</div>
      ...
      <div class="card" v-if="result.ai?.ratings">
        <h3 class="card-title">面相评级</h3>
        <div class="rating-list">
          <div v-for="part in faceParts" :key="part.key" class="rating-item">
    ...
    <LoadingOverlay :visible="loading" text="面相分析中..." />
  </div>
</template>
...
const faceParts = [
  { key: 'tianting', label: '天庭', color: '#e8b86d' },
  { key: 'meiyan', label: '眉眼', color: '#6db3e8' },
  { key: 'bixiang', label: '鼻相', color: '#e86d6d' },
  { key: 'kouxiang', label: '口相', color: '#6de88a' },
  { key: 'dige', label: '地阁', color: '#b86de8' },
]

const aiSections = [
  { key: 'zonglun', label: '面相总论' },
  { key: 'santing', label: '三停分析' },
  { key: 'wuguan', label: '五官详解' },
  { key: 'xingge', label: '性格分析' },
  { key: 'shiye', label: '事业运势' },
...
async function onSubmit() {
  ...
  loading.value = true
  try {
    const res: any = await http.post('/divination/face', {
...
async function fetchAiInterpretation() {
  ...
  await fetchAiStream('face', result.value, { image: imageBase64.value }, result.value?.id, {
    onParsed(parsed) {
